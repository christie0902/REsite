        ...
        return (
            <ul className={`ml-${level * 4} mb-4`}>
                {subcategories.map(subcategory => (
                    <li key={subcategory.id} className="ml-2 cursor-pointer hover:underline">
                        {subcategory.name}
    ...
    const renderParentCategories = () => {
        ...
        return parentCategories.map(parentCategory => (
            <div key={parentCategory.id} className="mb-6">
                <h2 className="font-bold text-yellow-500 cursor-pointer hover:underline">{parentCategory.name}</h2>

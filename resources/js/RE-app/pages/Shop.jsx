...
const Shop = () => {
  ...
  useEffect(()=>{
   ...
   const handleTagClick = (tagId) => {
    setSelectedTags(prevSelectedTags => {
      ...
      return isTagSelected ? prevSelectedTags.filter(id => id !== tagId) : [...prevSelectedTags, tagId];
    });
  };
  ...
  const loadData = async (page = 1, tags = selectedTags) => {
    const tagsQueryString = tags.length > 0 ? `&tags=${tags.join(',')}` : '';
    try {
      const response = await fetch(`/api/products?page=${page}${tagsQueryString}`);
  ...
  return (
    <>
      ...
      >
        ...
        <Tags onTagClick={handleTagClick} selectedTags={selectedTags} />
       
        <div className="container mx-auto px-4 py-5">
        {state.searchResults && <p>{`${state.searchResults.length} search results for "${state.searchQuery}"`}</p>}
         ...
         {showPages && products.meta.last_page > 1 && (
          <div className="pagination flex justify-center space-x-2 mb-10">
            {Array.from({ length: products.meta.last_page }, (_, i) => (
              <button
                key={i + 1}
                onClick={() => handlePageChange(i + 1)}
                className={`page-item ${
                  products.meta.current_page === i + 1 ? "active" : ""
                }`}
                disabled={products.meta.current_page === i + 1}
